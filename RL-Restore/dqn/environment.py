...
class MyEnvironment(object):
    def __init__(self, config):
        ...
        if self.is_train:
            ...
            self.data_test = f['data'][()]
            self.label_test = f['label'][()]
            f.close()
        ...
        for idx in range(12):
            ...
            with g.as_default():
                # load graph
                saver = tf.compat.v1.train.import_meta_graph(toolbox_path + 'tool%02d' % (idx + 1) + '.meta')
                # input data
            ...
            sess = tf.compat.v1.Session(graph=g, config=tf.compat.v1.ConfigProto(log_device_placement=True))
            with g.as_default():
