    ...
    def getEpiBatch(self, batch_size):
        ...
        for _ in range(batch_size):
            ...
            for m in range(len_cur):
                s_t_cur.append(cur_episode['s_t'][m])
                action_cur.append(cur_episode['action'][m])
                reward_cur.append(cur_episode['reward'][m])
        for k in range(self.stop_step):
            if len(reward[k]) > 0:
                s_t[k] = np.concatenate(s_t[k], axis=0).astype(np.float32)
            action[k] = np.array(action[k], dtype=np.int32)
            reward[k] = np.array(reward[k], dtype=np.float32)
    ...
    def getEpisode(self):  # return single episode
        ...
        while True:
            ...
            for k in range(idx_start, idx_end + 1):
                ...
                s_t_plus_1_list.append(s_t_plus_1)
            ...
            s_t_plus_1_array = np.array(s_t_plus_1_list)
            terminals_array = np.array(terminals_list)
            
            # Combine into a structured array
            output = np.array(list(zip(s_t_array, action_array, reward_array, s_t_plus_1_array, terminals_array)), 
                            dtype=[('s_t', 'f4', s_t_array.shape[1:]), 
                                  ('action', 'i4'), 
                                  ('reward', 'f4'), 
                                  ('s_t_plus_1', 'f4', s_t_plus_1_array.shape[1:]), 
                                  ('terminals', '?')])
            
            assert output['terminals'][-1]
            return output
